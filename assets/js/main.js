...
let navancer = document.querySelectorAll('li a');
...
navlinks.forEach((val) => {
	val.addEventListener("click", function () {
		...
		navlinks.forEach(link => {
			link.classList.remove("active_anc");
		});

		// Add the class to the clicked link
		val.classList.add("active_anc");
	});
});



const text = document.querySelector(".text_inner p");

text.innerHTML = text.innerText
	.split("")
	.map(
		(char, i) => `<div class"slef_info_div"><span style="transform:rotate(${i * 5.8}deg)">${char}</span></div>`
	)
	.join("");
